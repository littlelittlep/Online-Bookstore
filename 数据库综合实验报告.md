...
<center><strong><p style="font-size:25px">组长：61520407牛露</p></strong></center>
...
<center><strong><p style="font-size:25px">组员：71120130昝崇崇</p></strong></center>
...
​        用户可以浏览相关网页以查看店中书籍的相关信息，亦可按照书籍类别查看某类图书。用户注册并登录个人账号后即可将心仪书籍加入购物车，在购物车页面可对订单进行支付，支付后用户亦可取消订单。另外，用户可进行修改个人信息等操作。
...
<table border='1'>  
    ...
    <tr>  
        <td rowspan='10'>用户模块</td><td>注册</td>
    </tr> 
        <tr><td>登录</td> </tr> 
...
![image-20230128162618281](assets/image-20230128162618281.png)

后端：

![image-20230128162707586](assets/image-20230128162707586.png)

### 2. 实体类实现

在系统实现中，根据前面的数据库设计基础，本项目设计了五个实体类Address、Book、Inventory、Order、User，每个类包含对应的属性，具体设计如下所示：

#### 2.1 用户地址实体Address

![1674986139479](assets/1674986139479.png)

#### 2.2 书籍实体Book

![1674986223122](assets/1674986223122.png)

#### 2.3 库存实体Inventory

![1674986008325](assets/1674986008325.png)

#### 2.4 订单实体Order

![1674986056568](assets/1674986056568.png)

#### 2.5 用户实体User

![1674986107147](assets/1674986107147.png)

### 3. 功能函数实现

在系统实现中，用户功能函数主要包括注册新用户、用户登录、修改信息、查阅书籍、购买书籍等操作；管理员包括增加库存，管理用户信息等操作。

部分代码展示如下：

**用户登录**

![1675045472318](assets/1675045472318.png)

**更新库存**

![1675046476312](assets/1675046476312.png)

展示了对数据库几种主要的操作类型，相似类型的语句不进行展示（代码链接见文末）：
...
![image-20230128163150922](assets/image-20230128163150922.png)
...
![image-20230129123513337](assets/image-20230129123513337.png)
...
本次数据库课程大作业是我们四人团队所取得的一次非常丰硕的成果。在此之前，我们曾在暑期学校项目中有过开发经验，但这次作业却使我们对项目的分析和开发工作，尤其是数据库的设计与使用，有了更为清晰的认识。

在项目初始阶段，我们接到了题目并充分沟通，以及研究往届学长学姐模板作业，为项目确定了详细的需求。接着，我们进入了设计阶段，技术选型。在数据库设计方面，我们充分讨论并规范了关系型数据库中各个表的字段，包括字段名称、类型、主键、可空性以及字段含义等，这一详细的前期设计对于我们后期的开发工作具有了极大的指导意义。

在开发阶段，我们选用了Javaweb开发框架，并采用了目前非常流行的Springboot框架，数据库方面选用了关系型数据库MySQL，前端技术采用了业界使用比较广泛的React框架，同时使用GitHub进行协同管理，使用postman进行接口测试。尽管开发的过程充满了困难，但是我们团队内的成员相互支持，通过不断的讨论和修复bug，我们最终实现了预期的功能。

总的来说，本次数据库课程实验是对所学知识的一次有效实践，让我们更深入更全面地理解了数据库知识。
